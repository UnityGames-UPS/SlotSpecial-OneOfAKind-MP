...
public class BonusController : MonoBehaviour
{
  ...
  internal IEnumerator StartBlueJokerWheelGame(JokerResponse jokerResponse)
  {
    ...
    if (jokerResponse.blueRound == 3)
    {
      ...
      DOVirtual.DelayedCall(2f, () => StartCoroutine(StartGreenJokerWheelGame(SocketManager.resultData.payload.jokerResponse)));
    }
  ...
  private IEnumerator StartGreenJokerWheelGame(JokerResponse jokerResponse)
  {
    ...
    if (jokerResponse.greenRound == 3)
    {
      ...
      DOVirtual.DelayedCall(2f, () => StartCoroutine(StartRedJokerWheelGame(SocketManager.resultData.payload.jokerResponse)));
    }
  ...
  {
    if (SocketManager.resultData.payload.jokerResponse.payout[0] != 0)
    {
      double total = 0;
      foreach (double i in SocketManager.resultData.payload.jokerResponse.payout)
      {
        total += i;
      }
      double truncatedValue = Math.Floor(total * 100) / 100;
      Joker_End_UI.transform.GetChild(2).GetComponent<TMP_Text>().text = truncatedValue.ToString("F3");
      if (SocketManager.resultData.payload.totalWin == SocketManager.initFeatures.joker.payout[0])
      {
        Joker_End_UI.transform.GetChild(1).GetComponent<TMP_Text>().text = "YOU HAVE WON THE MINOR PRIZE";
      }
      else if (SocketManager.resultData.payload.totalWin > SocketManager.initFeatures.joker.payout[1] && SocketManager.resultData.payload.totalWin < SocketManager.initFeatures.joker.payout[2])
      {
        Joker_End_UI.transform.GetChild(1).GetComponent<TMP_Text>().text = "YOU HAVE WON THE MAJOR PRIZE";
      }
      else if (SocketManager.resultData.payload.totalWin > SocketManager.initFeatures.joker.payout[2])
      {
  ...
  private IEnumerator EndJoker()
  {
    ...
    yield return StartCoroutine(slotBehaviour.PlayWinningsAnimation(SocketManager.resultData.payload.totalWin));
    if (slotBehaviour.WasAutoSpinOn)
    {
      DOVirtual.DelayedCall(1.2f, slotBehaviour.wasAutoOn);
    }
