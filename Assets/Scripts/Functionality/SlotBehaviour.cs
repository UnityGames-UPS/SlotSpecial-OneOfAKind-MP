...
public class SlotBehaviour : MonoBehaviour
{
  ...
  private void Start()
  {
    ...
    if (TotalBetPlus_Button) TotalBetPlus_Button.onClick.AddListener(() => ChangeBet(true));

    if (TotalBetMinus_Button) TotalBetMinus_Button.onClick.RemoveAllListeners();
    if (TotalBetMinus_Button) TotalBetMinus_Button.onClick.AddListener(() => ChangeBet(false));
  ...
  private void PopulateAnimationSprites(int index = 0)
  {
    ...
    if (!IsFreeSpin)
    {
      startIndex = int.Parse(SocketManager.resultData.matrix[0][0]);
      lastIndex = SocketManager.resultData.payload.levelUpResponse.level;
    }
    else
    {
      startIndex = FreeSpinData.symbols[index];
      lastIndex = FreeSpinData.levelUp[index].level;
    }
  ...
  private IEnumerator TweenRoutine()
  {
    ...
    int resultID = int.Parse(SocketManager.resultData.matrix[0][0]);
    if (resultID == 0)
    {
      EmptyResult = true;
    }
    ...
    if (CheckFreeSpinTriggered())
    {
      IsFreeSpin = true;
      FreeSpinData = new();
      FreeSpinData.type = SocketManager.resultData.payload.freeSpinType;
      if (FreeSpinData.type.Contains("BLUE"))
      {
        FreeSpinData.isTriggered = SocketManager.resultData.payload.scatterBlueResponse.isTriggered;
        FreeSpinData.symbols = SocketManager.resultData.payload.scatterBlueResponse.symbols;
        FreeSpinData.payout = SocketManager.resultData.payload.scatterBlueResponse.payout;
        FreeSpinData.levelUp = SocketManager.resultData.payload.scatterBlueResponse.levelUp;
        FreeSpinData.booster = SocketManager.resultData.payload.scatterBlueResponse.booster;
        FreeSpinData.count = SocketManager.resultData.payload.scatterBlueResponse.count;
      }
      else
      {
        FreeSpinData.isTriggered = SocketManager.resultData.payload.scatterPurpleResponse.isTriggered;
        FreeSpinData.symbols = SocketManager.resultData.payload.scatterPurpleResponse.symbols;
        FreeSpinData.payout = SocketManager.resultData.payload.scatterPurpleResponse.payout;
        FreeSpinData.levelUp = SocketManager.resultData.payload.scatterPurpleResponse.levelUp;
        FreeSpinData.booster = SocketManager.resultData.payload.scatterPurpleResponse.booster;
        FreeSpinData.count = SocketManager.resultData.payload.scatterPurpleResponse.count;
        FreeSpinData.topSymbols = SocketManager.resultData.payload.scatterPurpleResponse.topSymbols;
        FreeSpinData.reTriggered = SocketManager.resultData.payload.scatterPurpleResponse.reTriggered;
      }
      ...
      if (SocketManager.resultData.payload.freeSpinType == "BLUE")
      {
      ...
      yield return new WaitForSeconds(1f);
      FreeSpinPopupCountText.text = SocketManager.resultData.payload.freeSpinType == "BLUE" ? "5" : "10";
    ...
    if (SocketManager.resultData.payload.jokerResponse.isTriggered)
    {
      featuredTriggered = true;
      StopAutoSpin(true);
      yield return new WaitForSeconds(1.5f);
      ...
      IsSpinning = false;
      DOVirtual.DelayedCall(2f, () => StartCoroutine(_bonusManager.StartBlueJokerWheelGame(SocketManager.resultData.payload.jokerResponse)));
      yield break;
    }

    if (SocketManager.resultData.payload.levelUpResponse.isLevelUp)
    {
      featuredTriggered = true;
      StopAutoSpin(true);
      TopUIToggle(false);
      int ResultValue = SocketManager.resultData.payload.levelUpResponse.level - int.Parse(SocketManager.resultData.matrix[0][0]);
      List<int> temp = new()
            {
                ResultValue
            };
    ...
    if (SocketManager.resultData.payload.boosterResponse.type != "NONE")
    {
      featuredTriggered = true;
      StopAutoSpin(true);
      TopUIToggle(false);
      BgController.SwitchBG(BackgroundController.BackgroundType.OrangeFR, SocketManager.resultData.payload.boosterResponse.multipliers, "MULTIPLIER");
      yield return new WaitForSeconds(0.2f);
      yield return BoosterActivatedAnimation();
      // Debug.Log("payout" + SocketManager.initUIData.paylines.symbols[resultID].payout + " currentTotalBet: " + currentTotalBet);
      StartCoroutine(StartMultiplierWheelGame(SocketManager.initUIData.paylines.symbols[resultID].payout * currentTotalBet, 0));
      IsSpinning = false;
      yield break;
    }

    if (SocketManager.resultData.payload.totalWin > 0 && !SocketManager.resultData.payload.jokerResponse.isTriggered && !SocketManager.resultData.payload.levelUpResponse.isLevelUp && SocketManager.resultData.payload.boosterResponse.type == "NONE" && SocketManager.resultData.payload.freeSpinType == "NONE")
    {
      checkPopups = false;
      WinAnimCoroutine = StartCoroutine(PlayWinningsAnimation(SocketManager.resultData.payload.totalWin));
      yield return new WaitUntil(() => checkPopups);
      BalanceAddition(SocketManager.resultData.payload.totalPayout);
      yield return TotalWinningsAnimation(SocketManager.resultData.payload.totalPayout);
    }
    else
    {
      ToggleButtonGrp(true);
  ...
  bool CheckFreeSpinTriggered()
  {
    return SocketManager.resultData.payload.overallFreeSpins > 0;
  }
  ...
  private void EndFreeSpin()
  {
    ...
    double payout = FreeSpinData.type.Contains("BLUE") ? FreeSpinData.payout : FreeSpinData.payout;
    if (payout > 0)
    {
  ...
  private IEnumerator StartFreeSpinGame()
  {
    ...
    freeSpinCount = FreeSpinData.type == "BLUE" ? 5 : 10;
    FSCount_Text.text = freeSpinCount.ToString();
    TWCount_Text.text = "0";
    FreeSpinsUI.DOFade(1, 0.5f);

    if (FreeSpinData.type == "BLUE")
    {
    ...
    else
    {
      ...
      PopulateTopSymbolUI(FreeSpinData.topSymbols[0]);
      TopPurpleUI.DOFade(1, 0.5f);
    }
    yield return new WaitForSeconds(1f);
    for (int i = 0; i < FreeSpinData.symbols.Count; i++)
    {
      yield return FSTweenRoutine(i);
      yield return new WaitForSeconds(SpinDelay);
    }

    if (FreeSpinData.payout > 0)
    {
      CanvasGroup FSendUI = FreeSpinPopupUI.GetChild(1).GetComponent<CanvasGroup>();
      double truncatedValue = Math.Floor(SocketManager.resultData.payload.totalWin * 100) / 100;
  ...
  private IEnumerator FSTweenRoutine(int index)
  {
    ...
    int resultID = FreeSpinData.symbols[index];
    if (resultID == 0)
    {
      EmptyResult = true;
    }
    ...
    if (FreeSpinData.type == "PURPLE")
    {
      ...
      if (!check && FreeSpinData.reTriggered[index] == 1)
      {
        Debug.Log("Refilling top purple symbols");
        PopulateTopSymbolUI(FreeSpinData.topSymbols[index + 1]);
      }
    }
    yield return new WaitForSeconds(1f);

    //check if free spin count increased and do an animation
    if (FreeSpinData.count[index] > freeSpinCount)
    {
      int count = FreeSpinData.count[index] - freeSpinCount;
    ...
    if (FreeSpinData.levelUp[index].isLevelUp)
    {
      int ResultValue = FreeSpinData.levelUp[index].level - resultID;
      List<int> temp = new()
            {
                ResultValue
            };
    ...
    else if (FreeSpinData.booster[index].type != "NONE")
    {
      BgController.SwitchBG(BackgroundController.BackgroundType.OrangeFR, FreeSpinData.booster[index].multipliers, "MULTIPLIER");
      yield return new WaitForSeconds(0.2f);
      yield return BoosterActivatedAnimation();
    ...
    else if (FreeSpinData.symbols[index] != 0)
    {
      FreeSpinWinningsTextAnimation(SocketManager.initUIData.paylines.symbols[FreeSpinData.symbols[index]].payout * currentTotalBet);
      yield return TotalWinningsAnimation(SocketManager.initUIData.paylines.symbols[FreeSpinData.symbols[index]].payout * currentTotalBet, true, false);
      yield return new WaitForSeconds(1.5f);
    }
  }
  ...
  private IEnumerator StartLevelBoosterWheelGame(BackgroundController.BackgroundType type, bool FS = false, int i = 0)
  {
    ...
    if (FS)
    {
      stopAt = FreeSpinData.levelUp[i].level - FreeSpinData.symbols[i];
      Debug.Log("FS level id: " + FreeSpinData.levelUp[i].level + " symbol id: " + FreeSpinData.symbols[i]);
    }
    else
    {
      stopAt = SocketManager.resultData.payload.levelUpResponse.level - int.Parse(SocketManager.resultData.matrix[0][0]);
    }
    ...
    if (!FS)
      this.ResultImage.sprite = SlotSymbols[SocketManager.resultData.payload.levelUpResponse.level];
    else
      this.ResultImage.sprite = SlotSymbols[FreeSpinData.levelUp[i].level];

    yield return new WaitForSeconds(2f);
    double payout = 0;
    if (!FS)
    {
      payout = SocketManager.initUIData.paylines.symbols[SocketManager.resultData.payload.levelUpResponse.level].payout * currentTotalBet;
      if (SocketManager.resultData.payload.boosterResponse.type != "NONE")
      {
        ...
        yield return new WaitForSeconds(0.2f);
        yield return BoosterActivatedAnimation();
        StartCoroutine(StartMultiplierWheelGame(SocketManager.initUIData.paylines.symbols[SocketManager.resultData.payload.levelUpResponse.level].payout * currentTotalBet, 0));
    ...
    {
      payout = SocketManager.initUIData.paylines.symbols[FreeSpinData.levelUp[i].level].payout * currentTotalBet;
      if (FreeSpinData.booster[i].type != "NONE")
      {
        yield return TotalWinningsAnimation(payout, false);
        yield return new WaitForSeconds(2f);
        CloseSlotWinningsUI();
        BgController.SwitchBG(BackgroundController.BackgroundType.OrangeFR, FreeSpinData.booster[i].multipliers, "MULTIPLIER");
        yield return new WaitForSeconds(0.2f);
        yield return BoosterActivatedAnimation();
        WinningsTextAnimation(0, false);
  ...
  private IEnumerator StartMultiplierWheelGame(double basePayout, int MultiplierIndex, bool FS = false, int i = 0)
  {
    ...
    if (MultiplierIndex != -1)
    {
      if (!FS)
      {
        stopper.stopAT = SocketManager.resultData.payload.boosterResponse.multipliers[MultiplierIndex].ToString(); //TELLING THE STOPPER THE LOCATION TO STOP
      }
      else
      {
        stopper.stopAT = FreeSpinData.booster[i].multipliers[MultiplierIndex].ToString();
      }
    ...
    if (FS)
    {
      if (FreeSpinData.booster[i].type == "EXHAUSTIVE" && MultiplierIndex != -1)
      {
        yield return new WaitForSeconds(1f);
        if (FreeSpinData.booster[i].multipliers[MultiplierIndex] != FreeSpinData.booster[i].multipliers[^1])
        {
    ...
    {
      if (SocketManager.resultData.payload.boosterResponse.type == "EXHAUSTIVE" && MultiplierIndex != -1)
      {
        yield return new WaitForSeconds(1f);
        if (SocketManager.resultData.payload.boosterResponse.multipliers[MultiplierIndex] != SocketManager.resultData.payload.boosterResponse.multipliers[^1])
        {
    ...
    {
      BgController.SwitchBG(BackgroundController.BackgroundType.Base); //ENDING SIMPLE MULTIPLAYER GAME
      if (multiplierWinnings != SocketManager.resultData.payload.totalWin)
      {
  ...
  {
    double.TryParse(TWCount_Text.text, out double TWCount);
    double total = amount + TWCount;

    DOTween.To(() => TWCount, (val) => TWCount = val, total, 0.8f)
    .OnUpdate(() =>
    {
